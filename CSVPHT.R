...
ex=ENP2[which(ENP1$GROUP=="CS" | ENP1$GROUP=="PHT"),]
WHYF=ENP1[which(ENP1$GROUP=="CS" | ENP1$GROUP=="PHT"),]
...
hay=function(x) {
  ...
  say=function(i){
    ...
    perf.plsda.train <- perf(plsda.train, validation = "Mfold", folds = 6, auc = F, progressBar = T, nrepeat = 1, dist="mahalanobis.dist")
    set.seed(1)
    tune.splsda.train <- tune.splsda(X, Y, ncomp = min(which(perf.plsda.train$error.rate$BER[,1]==min(perf.plsda.train$error.rate$BER[,1]))), validation = "Mfold", folds = 6, progressBar = T, dist = "mahalanobis.dist", measure = "BER", nrepeat = 1, scale = F)
...
conf.mitch=function(x) {
  mitch[[x]][[7]][[1]]+mitch[[x]][[6]][[1]]+mitch[[x]][[5]][[1]]+mitch[[x]][[4]][[1]]+mitch[[x]][[3]][[1]]+mitch[[x]][[2]][[1]]+mitch[[x]][[1]][[1]]
}
...
set.seed(1)  #for keeping perf the same as with lapply
perf.plsda.select <- perf(plsda.select, validation = "Mfold", folds = 7, auc = F, progressBar = T, nrepeat = 50, dist="mahalanobis.dist")
ncomp=perf.plsda.select$choice.ncomp[2,]
set.seed(1)
tune.splsda.select <- tune.splsda(X, Y, ncomp = ncomp, validation = "Mfold", folds = 7, progressBar = T, dist = "mahalanobis.dist", measure = "BER", nrepeat = 50, scale = F)
...
CSVPHT1=list(mitch, all.BER, checker, checke2, perm1, WHYF, allMC, allCC, plsda.select, splsda.select, perf.plsda.select, tune.splsda.select, rich)
...
X=ex
set.seed(1)  #for keeping perf the same as with lapply
cvfit1 = cv.glmnet(X, Y, family = "binomial", alpha=1, nfolds = 7, type.measure = "class", standardize=FALSE)
rich=coef(cvfit1, s = "lambda.min")
rich=rich[-1,]
...
CSVPHT3=list(mitch, all.BER, checker, checke2, perm1, WHYF, allMC, allCC, cvfit1, rich)
...
hay=function(x) {
  ...
  say=function(i){
    ...
    perf.plsda.train <- perf(plsda.train, validation = "Mfold", folds = 6, auc = F, progressBar = T, nrepeat = 1, dist="mahalanobis.dist")
    set.seed(1)
    tune.splsda.train <- tune.splsda(X, Y, ncomp = min(which(perf.plsda.train$error.rate$BER[,1]==min(perf.plsda.train$error.rate$BER[,1]))), validation = "Mfold", folds = 6, progressBar = T, dist = "mahalanobis.dist", measure = "BER", nrepeat = 1, scale = T)
    ncomp=ceiling(min(which(tune.splsda.train$error.rate==min(tune.splsda.train$error.rate)))/nrow(tune.splsda.train$error.rate))
...
ncomp=perf.plsda.select$choice.ncomp[2,]
set.seed(1)
tune.splsda.select <- tune.splsda(X, Y, ncomp = ncomp, validation = "Mfold", folds = 7, progressBar = T, dist = "mahalanobis.dist", measure = "BER", nrepeat = 50, scale = T)
ncomp=tune.splsda.select$choice.ncomp$ncomp
splsda.select <- splsda(X, Y, ncomp = ncomp, scale = T, keepX = tune.splsda.select$choice.keepX)
rich=vip(splsda.select)
rich[which(rowSums(rich)>0),]

CSVPHT5=list(mitch, all.BER, checker, checke2, perm1, WHYF, allMC, allCC, plsda.select, splsda.select, perf.plsda.select, tune.splsda.select, rich)
...
hay=function(x) {
  ...
  say=function(i){
    ...
    set.seed(1)  #for keeping perf the same as with lapply
    cvfit1 = cv.glmnet(X, Y, family = "binomial", alpha=1, nfolds = 6, type.measure = "class")
    #fit = glmnet(X, Y, family = "binomial", alpha=1, standardize = F, lambda = cvfit1$lambda.min)
    jack=WHYF[test,]
    ENP=ex[test,]
    dataset_test=ENP
...
X=ex
set.seed(1)  #for keeping perf the same as with lapply
cvfit1 = cv.glmnet(X, Y, family = "binomial", alpha=1, nfolds = 7, type.measure = "class", standardize=T)
rich=coef(cvfit1, s = "lambda.min")
rich=rich[-1,]
rich=rich[which(rich>0 | rich<0)]
rich=as.matrix(rich[order(rich)])
...
CSVPHT7=list(mitch, all.BER, checker, checke2, perm1, WHYF, allMC, allCC, cvfit1, rich)


#ASCA: NOT ENOUGH SAMPLES FOR GROUP=CS & CLUS==1!
